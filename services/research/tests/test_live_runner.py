...
def test_run_once_evaluates_each_new_candle_only_once(cfg, monkeypatch):
    ...
    r = resample(e, "1h")
    monkeypatch.setattr(runner, "load_for", lambda inst, cfg, until=None: (e, r))
    published, written = [], []
    seen = {}
...
def test_run_once_publishes_only_fresh_signals(cfg, monkeypatch):
    ...
    monkeypatch.setattr(runner, "evaluate_instance", lambda cfg, inst, ed, rd: runner.Evaluation(inst.id, ed.index[-1], runner.build_signal(inst, ed.index[-1], d), "signal"))
    published = []
