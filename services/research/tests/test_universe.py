...
def test_render_copies_the_template_per_symbol(repo_root):
    ...
    text = render_instances(tpl, ["ETHUSDT", "1000SATSUSDT"])
    assert instance_id(tpl, "1000SATSUSDT") == "s1_1000sats_15m"
    parsed = yaml.safe_load(text)
    assert [p["id"] for p in parsed] == ["s1_eth_15m", "s1_1000sats_15m"]
    ...
    raw["symbols"] += ["ETHUSDT", "1000SATSUSDT"]
...
class FakeMarket:
    ...
    def load_markets(self):
        ...
        return {f"{b}/USDT": mk(b) for b in ["BTC", "ETH", "USDC", "TINY"]} | {"ETH/BTC": {"id": "ETHBTC", "base": "ETH", "quote": "BTC", "spot": True, "active": True}}

    def fetch_tickers(self):
        return {"BTC/USDT": {"quoteVolume": 600e6}, "ETH/USDT": {"quoteVolume": 200e6}, "USDC/USDT": {"quoteVolume": 900e6}, "TINY/USDT": {"quoteVolume": 1e6}}

    def publicGetTickerBookTicker(self):
        return [{"symbol": "BTCUSDT", "bidPrice": "100", "askPrice": "100.01"}, {"symbol": "ETHUSDT", "bidPrice": "10", "askPrice": "10.002"}]

    def publicGetKlines(self, params):
        vol = {"BTCUSDT": 500e6, "ETHUSDT": 150e6}[params["symbol"]]
        day = 86_400_000
...
def test_scan_measures_lookback_volume_and_spread():
    ...
    assert set(cs) == {"BTCUSDT", "ETHUSDT"}  # USDC excluded, TINY under the ticker prefilter, ETH/BTC wrong quote
    assert cs["ETHUSDT"].avg_volume_usd == pytest.approx(150e6)
    assert cs["ETHUSDT"].history_days == 30
    assert cs["ETHUSDT"].spread_pct == pytest.approx(0.02, abs=1e-4)
...
def test_refresh_end_to_end(tmp_path, repo_root):
    ...
    assert "ETHUSDT" in cfg.symbols and any(s.id == "s1_eth_15m" for s in cfg.strategies)
    # a second refresh with ETH gone drops it again, hand-written instances untouched
    class Quiet(FakeMarket):
        def fetch_tickers(self):
            return {"BTC/USDT": {"quoteVolume": 600e6}}
    r2 = refresh(p, ex=Quiet(), sleep=False)
    assert r2.dropped == ["ETHUSDT"] and r2.new_symbols == []
