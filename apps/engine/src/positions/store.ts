...
export class PgPositionStore implements PositionStore {
  ...
  async insert(p: PositionRecord) {
    ...
    await this.sql`
      ...
      VALUES (${p.id}, ${this.mode}, ${p.signal_id}, ${p.strategy_id}, ${p.symbol}, ${p.timeframe}, 'long', ${s.entry}, ${s.qty},
        ...
        ${s.bars}, ${s.state}, ${p.opened_at}, ${this.sql.json(p.exit_params as never)},
        ${this.sql.json({ regime_tf: p.regime_tf, invalidation_level: p.invalidation_level, risk_amount: p.risk_amount, fee_pct: p.fee_pct, events: s.events } as never)})`;
  }

  async update(p: PositionRecord) {
    const s = p.state;
    await this.sql`
      UPDATE positions SET remaining_qty = ${s.remaining_qty}, sl_price = ${s.sl}, tp_price = ${s.tp}, tp1_done = ${s.tp1_done},
        highest_high = ${s.highest_high}, lowest_low = ${s.lowest_low}, bars_held = ${s.bars}, state = ${s.state},
        meta = meta || ${this.sql.json({ events: s.events } as never)}
      WHERE id = ${p.id}`;
  }
  ...
  async markSignalAccepted(signalId: string, positionId: string, actualEntry: number, slippagePct: number) {
    await this.sql`UPDATE signals SET mode = ${this.mode}, position_id = ${positionId}, actual_entry = ${actualEntry},
      slippage_pct = ${slippagePct} WHERE id = ${signalId}`;
  }
  async markSignalRejected(signalId: string, reason: string) {
    await this.sql`UPDATE signals SET mode = ${this.mode}, outcome = 'rejected', reject_reason = ${reason} WHERE id = ${signalId}`;
  }
  ...
  async recentOutcomes() {
    const rows = await this.sql<{ realized_r: string; closed_at: Date }[]>`
      SELECT realized_r, closed_at FROM signals WHERE mode = ${this.mode} AND closed_at IS NOT NULL
      ORDER BY closed_at DESC LIMIT 50`;
    return rows.map((r) => ({ realized_r: Number(r.realized_r), closed_at: r.closed_at }));
  }
}
...
export class MemoryPositionStore implements PositionStore {
  ...
  accepted = new Map<string, { positionId: string; actualEntry: number }>();
  ...
  async loadOpen() {
    return [...this.positions.values()].filter((p) => p.state.state !== "closed");
  }
  async markSignalAccepted(signalId: string, positionId: string, actualEntry: number) {
    this.accepted.set(signalId, { positionId, actualEntry });
  }
  async markSignalRejected(signalId: string, reason: string) {
